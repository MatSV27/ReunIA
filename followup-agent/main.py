...
def followup_run(request):
    ...
    for chat_id, docs in by_chat.items():
        try:
            ...
            # One chat's bad Gemini output/parse error shouldn't stop every other
            # chat from getting its digest -- log it and keep going.
            logging.exception(f"followup_run: failed to process chat {chat_id}, skipping")
            chats_failed += 1

    return (
        f"Sent {digests_sent} digest(s), acted on {tasks_acted_on} task(s), "
        f"{chats_failed} chat(s) failed",
        200,
    )
