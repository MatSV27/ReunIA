...
2. For each chat's task list, Follow-up Agent (Gemini, via ADK) reasons over the *entire* list at once — not one task in isolation — and returns: a per-task decision (`remind` / `escalate` / `skip`) plus **one** consolidated digest message. It's also asked to call out cross-task patterns (e.g. the same owner behind on multiple items) when there genuinely is one. The agent has a real tool, `get_task_events(task_id)` (queries `tasks/{id}/events`), which it can call itself for a task whose `reminder_count` alone is ambiguous — e.g. two tasks reminded twice each look identical by count, but one whose reminders are spread over three weeks is genuinely stuck, while one reminded twice in the last two days is still fresh. The tool lets the agent tell those apart from real timestamps instead of a flat threshold, so `reminder_count >= 2` is a *hint* to investigate, not an automatic escalation rule.
